...
pub struct BrowseOpts {
    ...
    /// Roon API hierarchy. Defaults to "browse" when None. Set to e.g.
    /// "now_playing" to use the now-playing action menu (which exposes
    /// Love/Unlove that the standard "browse" menu does not).
    /// Patched into the upstream crate by the Roon AI fork.
...
pub struct LoadOpts {
    ...
    /// Roon API hierarchy. Defaults to "browse" when None. Must match the
    /// hierarchy used in the preceding browse() call within the same session.
    /// Patched into the upstream crate by the Roon AI fork.
    #[serde(skip_serializing)]
    pub hierarchy: Option<String>,
}
...
impl Browse {
    ...
    pub async fn browse(&self, opts: &BrowseOpts) -> Option<usize> {
        ...
        // Resolve hierarchy from the patched opts field; default to "browse"
        // for backwards compatibility with all existing callsites.
        let hierarchy = opts.hierarchy.clone().unwrap_or_else(|| "browse".to_string());
        let mut opts = serde_json::to_value(opts).ok()?;

        opts["hierarchy"] = hierarchy.into();
